...
export default function DigitalCard({ partner, websiteData }: DigitalCardProps) {
  ...
  const shareableCardLink = `${window.location.origin}/site/${partner.id}/card`;
  const whatsappShareText = `Check out my digital card: ${shareableCardLink}`;
    ...
    <div className="flex items-center justify-center min-h-full p-4">
      <div className="w-full max-w-sm mx-auto bg-card rounded-xl shadow-lg overflow-hidden border">
        ...
        <div className="grid grid-cols-2 gap-px bg-border">
          <a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappShareText)}`} target="_blank" rel="noopener noreferrer">
